...
2. 保持提交原子化，提交信息清晰
3. 提交前执行本地构建与测试
4. 发起 Pull Request，描述变更动机与测试结果

### 提交信息建议

可参考 Conventional Commits：
...
- [ ] 增加统一 `.env.example` 配置模板
- [ ] 前端增加会话管理与历史记录
- [ ] 增加更多 MCP 工具（地图、文件系统、数据库）
- [ ] 接入可观测性（日志聚合、链路追踪、指标）
- [ ] 增加自动化测试覆盖（后端 + 前端）
- [ ] 发布英文 README 与 API 文档站点

---

## 安全说明

- 不要在仓库中提交真实 API Key、数据库密码。
- 生产环境建议使用环境变量或密钥管理服务注入配置。
- `tmp/` 为运行产物目录，建议保持在版本控制之外。

---

## 许可证

当前仓库未声明 License。若计划开源发布，建议添加 `LICENSE`（如 MIT / Apache-2.0）。
